...
export function CategoryIconGrid({
  ...
  return (
    <section className="my-3 sm:my-4" aria-label="Browse by category">
      <div className="flex items-center gap-2 sm:gap-3 overflow-x-auto pb-2 scrollbar-none px-1">
        {CATEGORY_ITEMS.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center gap-1.5 p-2 sm:p-2.5 rounded-2xl transition-all duration-150 shrink-0 w-[76px] sm:w-[90px] text-center group ${
                isSelected
            ...
            >
              <div className={`w-11 h-11 sm:w-13 sm:h-13 rounded-xl ${item.bg} flex items-center justify-center text-2xl sm:text-3xl select-none group-hover:scale-110 transition-transform`}>
                {item.emoji}
              </div>
